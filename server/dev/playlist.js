...
router.get('/moviesinplaylist/:video_id/', async function (req, res) {
    ...
    try{
        ...
        let user = req.user;
        let movie = await queryDB(getPlaylistsByMovie, [video_id, user.user_id]);

        if (movie.length === 0) {
            res.status(400).send([req.params,"ERROR 400: There is no videos in playlist with this ID",video_id, movie,user.user_id]);
            return;
        }
        ...
        res.status(400).send([req.params,"ERROR 400: There is no videos in playlist with this ID"]);
    }
...
router.get("/guest", async function (req, res) {
    ...
    try{
        ...
        const guestUserPlaylist = await queryDB(getPlaylistByGuestId, [user.user_id]);
